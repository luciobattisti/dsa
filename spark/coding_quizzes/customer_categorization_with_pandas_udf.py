...
from pyspark.sql.functions import udf, sum as spark_sum, col, pandas_udf
...
def categorize_customer(spending_total: pd.Series) -> pd.Series:
    def foo(x: int)->str:
        if x < 200:
            return "Low Spender"
        elif x < 500:
    ...
    return spending_total.apply(lambda x: foo(x))
...
category_df = (
    aggregate_df.withColumn(
        "category",
        categorize_customer(col("total_spending"))
    ).select(
        col("customer"),
        col("total_spending"),
        col("category")
